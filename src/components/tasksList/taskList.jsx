...
const TaskList = ( {title,type,tasks} ) => {
    ...
    return ( 
        <div className={styles.container}>
            ...
            {type === "overdue" ? tasks.filter((task) => isPast(new Date(task.dueDate))).map(task => (<TaskCard task={task} key={task._id}/>)) :tasks.filter((task) => task.status === type && !isPast(new Date(task.dueDate))).map(task => (<TaskCard task={task} key={task._id}/>))}</div>}
        </div>
     );
}
